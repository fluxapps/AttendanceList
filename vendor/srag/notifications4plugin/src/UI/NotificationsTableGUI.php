...
class NotificationsTableGUI extends TableGUI {
	 ...
	 */
	public function __construct(PluginInterface $plugin, CtrlInterface $parent, $parent_cmd, callable $getNotifications, callable $getNotificationsCount) {
		$this->plugin = $plugin;
	...
	protected function getColumnValue($column, $row, $format = self::DEFAULT_FORMAT) {
		...
		switch ($column) {
			case "languages":
				$value = implode(", ", array_map(function (NotificationLanguage $language) {    return $language->getLanguage();
}, $value));
				break;
	 ...
	 * @inheritdoc
	 */
	public function getSelectableColumns2() {
		$columns = [
			"title" => "title",
    ...
    return ["id" => $key, "default" => true, "sort" => $key !== "languages"];
}, $columns);
