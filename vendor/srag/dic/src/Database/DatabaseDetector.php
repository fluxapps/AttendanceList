...
class DatabaseDetector extends AbstractILIASDatabaseDetector
{
     ...
     * @inheritDoc
     */
    public function createOrUpdateTable($table_name, array $columns, array $primary_columns)/*: void*/
    {
     ...
     * @inheritDoc
     */
    public function multipleInsert($table_name, array $columns, array $values)/*:void*/
    {
        if (empty($columns) || empty($values)) {
            return;
        }

        $this->manipulate('INSERT INTO ' . $this->quoteIdentifier($table_name) . ' (' . implode(',', $columns) . ') VALUES ' . implode(',', array_map(function (array $values2) {    return '(' . implode(',', array_map(function (array $value) {        return $this->quote($value[0], $value[1]);
    }, $values2)) . ')';
}, $values)));
    }
