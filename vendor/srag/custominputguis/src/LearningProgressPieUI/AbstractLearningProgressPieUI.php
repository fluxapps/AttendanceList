...
use ilLPStatus;
use srag\CustomInputGUIs\AttendanceList\CustomInputGUIsTrait;
...
abstract class AbstractLearningProgressPieUI {
	...
	public function render() {
		...
		if (count($data) > 0) {
			...
			$data = array_map(function ($status) use($data) {    return ["color" => self::LP_STATUS_COLOR[$status], "title" => $this->getText($status), "value" => $data[$status]];
}, self::LP_STATUS);
