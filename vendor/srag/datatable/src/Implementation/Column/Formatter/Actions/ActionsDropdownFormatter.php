...
class ActionsDropdownFormatter extends DefaultFormatter implements ActionsFormatter
{
    ...
    public function formatRowCell(Format $format, $actions, Column $column, RowData $row, $table_id)
    {
        return self::output()->getHTML(self::dic()->ui()->factory()->dropdown()
            ->standard(array_map(function (Component $button) use($format, $row, $table_id) {    if ($button instanceof Shy) {        return Closure::bind(function (Format $format, RowData $row, $table_id) {            if (!empty($this->action) && empty($this->triggered_signals["click"])) {                $this->action = $format->getActionUrlWithParams($this->action, [Table::ACTION_GET_VAR => $row->getRowId()], $table_id);
            }
            return $this;
        }, $button, Button::class)($format, $row, $table_id);
    }
    if ($button instanceof StandardInterface) {
        return Closure::bind(function (Format $format, RowData $row, $table_id) {
