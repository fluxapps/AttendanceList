...
class StaticDataFetcher extends AbstractDataFetcher
{
     ...
     * @param string   $id_key
     */
    public function __construct(array $data, $id_key)
    {
        parent::__construct();
    ...
    {
        $data = array_filter($this->data, function (stdClass $data) use($settings) {    $match = true;    foreach ($settings->getFilterFieldValues() as $key => $value) {        if (!empty($value)) {            switch (true) {                case is_array($value):                    $match = in_array($data->{$key}, $value);                    break;
    ...
    return $match;
        ...
        usort($data, function (stdClass $o1, stdClass $o2) use($settings) {
