...
class srGenericMultiInputGUI extends ilFormPropertyGUI {
	...
	public function render($iterator_id = 0, $clean_render = false) {
		$tpl = new ilTemplate("tpl.prop_generic_multi_line.html", true, true, 'Customizing/global/plugins/Services/Repository/RepositoryObject/AttendanceList');
...
//		$tpl->addCss($this->getTemplateDir() . '/templates/css/multi_line_input.css');
		...
		if ($this->getMulti()) {
			...
			$tpl->addJavascript(ilAttendanceListPlugin::getInstance()->getDirectory() . '/templates/js/generic_multi_line_input.js');
			$output .= '<script type="text/javascript">$("#' . $this->getFieldId() . '").multi_line_input('
				. json_encode($this->input_options) . ', '
				. json_encode(array('limit' => $this->limit, 'sortable' => $this->multi_sortable, 'locale' => $lng->getLangKey()))
				. ')</script>';
		}
